...
import CovHeaderCard from "../components/covheaderCard";
...
const Users = () => {
  ...
  const { settingsName, settingsStatus } = useSelector(
  ...
  const handleSubmit = (e) => {
    ...
    if (validationResult.isValid) {
      ...
      if (methodEdit == "PUT") {
        url = `Users/PUT`;
        const roleID = userRole.find(
          (item) => item.roleName === _data.roleName
        ).roleID;
      ...
      getFetch(url, "POST", _data)
        .then((data) => {
          if (data.statusCode === 200 || data.statusCode === 204) {
            ...
            const userId =
              data.userID || safeEntityData[0]?.userId || formData.userID;
            ...
            const updatedEntityData = safeEntityData.map((item) => {
              const matchedEntity = userDepart.find(
                (dep) => dep.entityName == item.entityName
              );
              return {
                ...item,
                userId: userId,
            ...
            //auditLog(userId, "User Admin");
            const filteredEntityData = updatedEntityData.filter(
              (item) => item.docEntry === undefined || item.docEntry === null
            );
            ...
            navigate("/app/error", {
              state: {
                ...
                errorCode: data.statusCode,
                errorName:
                  data.errorName || "No additional information available.",
              },
            });
          }
        })
        .catch((error) => {
          ...
          navigate("/app/error", {
            state: {
              ...
              errorCode: 500,
              errorName:
                error.errorName || "No additional information available.",
            },
          });
        });
    } else {
                    ...
                    <Box sx={{ marginTop: "10px" }}>
                      <Autocomplete
                        ...
                        value={
                          formData.roleName
                            ? userRole.find(
                                (item) => item.roleName === formData.roleName
                              )
                            : formData.roleName
                        }
                      ...
                      {formData.roleName === "3" ? (
                        <Autocomplete
                          ...
                          value={
                            formData.userName
                              ? supplierData.find(
                                  (item) => item.supName === formData.userName
                                )
                              : formData.userName
                          }
                          onChange={(event, value) => {
                      ...
                      settingsName.status != "N" && (
                        ...
                        <Box sx={{ marginTop: "10px" }}>
                          <TextField
                            ...
                            name="entityDefWhs"
                            value={
                              Array.isArray(entityData) && entityData.length > 0
                                ? entityData.find((e) => e.entityDefWhs)
                                    ?.entityDefWhs || ""
                                : ""
                            }
                            InputProps={{
